...
const Emergency = () => {
  ...
  const { data: emgData, isLoading, error } = useQuery({
    queryKey: ['emergency', customId],
    queryFn: async () => (await api.get(`/emergency/${customId}`)).data,
    enabled: !!customId,
    retry: false,
  });

  if (isLoading) {
    return <div className="flex items-center justify-center p-20 text-teal-400"><Activity className="animate-spin" size={48}/></div>;
  }
  ...
  if (error || !emgData) {
    return (
      <div className="flex flex-col items-center justify-center p-20">
        ...
        <p className="text-slate-400 mt-2">Could not retrieve emergency record.</p>
      </div>
    );
  }
